...
clock_time_t coap_check_rto_state(clock_time_t rto, clock_time_t oldrto,uip_ipaddr_t *addr){
	...
	//Delay Gradient Calculation begin
	coap_rtt_estimations_t *t = NULL;
	for (t = (coap_rtt_estimations_t*)list_head(rtt_estimations_list); t; t = t->next){
		    if (uip_ipaddr_cmp(addr, &t->addr)){
			printf("Checker\n");
	...
	//int xyz = avg_delta_min/1000;
	//printf("xyz = %d",xyz);
	int pbf = 1 - exp(-avg_delta_min);
	...
	printf("Measured RTT:%lu\n", (unsigned long) rtt);
		  for (t = (coap_rtt_estimations_t*)list_head(rtt_estimations_list); t; t = t->next){
			...
			if((t->rtt_count+1) % 5 == 0){
				...
				t->rtt_count=t->rtt_count+1;
				printf("delta min check %lu %lu %li %d\n",t->rttmin,temp,t->delta_min,t->rtt_count);

				temp = t->rttmax;
				if(t->rttmax == 0)
					t->rttmax = rtt;
				else if(rtt > t->rttmax)
					t->rttmax = rtt;
